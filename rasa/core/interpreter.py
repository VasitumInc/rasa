...
class RasaNLUInterpreter(NaturalLanguageInterpreter):
    ...
    async def parse(
        ...
        for x in result["entities"]:
            try:
                text = text.lower()
                text = text.replace(str(x["value"]),x["entity"])
            except:
                pass    
        print("***********",text)
        body = {"message":text}
        intent = rq.post("http://localhost:5057/predict_intent",json=body)
        intent = intent.json()
        print(intent,"______",result)
        intent["confidence"] = intent["confidence"]/10000
        result["intent"] = intent
        entities = []
        for x in result["entities"]:
            if x["entity"] == "skill" and x['extractor'] == "CRFEntityExtractor":
                pass
            elif x["entity"] == "company" and x['extractor'] == "CRFEntityExtractor":
                 pass
            else:
                entities.append(x)
